...
export class DictionaryNew {
  ...
  createDictionariesNew(tripleList) {
    const tripleListcopy = JSON.parse(JSON.stringify(tripleList));
    tripleListcopy.forEach((triple) => {
      let subject = triple[0];
      let predicate = triple[1];
      let object = triple[2];
      this.#inputTripleRedone(subject, predicate, object);
    });
  ...
  #inputTripleRedone(subject, predicate, object) {
    this.S.push(subject);
    this.P.push(predicate);
    this.O.push(object);
  }
  ...
  #intersectArrays() {
    const map = this.#distinctMapFromArray(this.S)
